...
class Label(LabelBase):
    ...
    def _create_background_box(self, lines: int, y_offset: int) -> TileGrid:
        ...
        # draw a "loose" bounding box to include any ascenders/descenders.
        else:
    ...
    def _set_background_color(self, new_color: Optional[int]) -> None:
        ...
        if not self._added_background_tilegrid:  # no bitmap is in the self Group
            ...
            if (
                ...
                and (
                    self._bounding_box[2] +
                    self._padding_left + self._padding_right > 0
                )
                and (
                    self._bounding_box[3] + self._padding_top +
        ...
        for character in new_text:
            ...
            if self._label_direction in ("LTR", "RTL"):
                ...
                if self._label_direction == "LTR":
                    ...
                    left = max(
                        left, abs(x) + glyph.shift_x, abs(x) +
                        glyph.width + glyph.dx
                    )
                    if x == 0:
                        if right is None:
                            right = glyph.dx
                ...
                bottom = max(bottom, y + glyph.height,
                             y + glyph.height + glyph.dy)
                right = max(
                ...
                bottom = max(bottom, y + glyph.width +
                             glyph.dx, y + glyph.shift_x)
                right = max(right, x + glyph.height)
